    ...
    async.mapSeries(
      ...
      function (sessionData, callback) {
        ...
        var taskList = taskListsBuilder.deploy(
          bundlePath, env,
          deployCheckWaitTime, appName, enableUploadProgressBar, enableSharpBinaryFix, noBundleDelete);
        taskList.run(session, function (summaryMap) {
          callback(null, summaryMap);
        });
      },
      whenAfterDeployed(buildLocation, noBundleDelete)
    );
  }
...
function whenAfterDeployed(buildLocation, noBundleDelete) {
