...
Actions.prototype.deploy = function() {
  ...
  var enableSharpBinaryFix = this.config.enableSharpBinaryFix;
  var noBundleDelete = this.config.noBundleDelete;
  var noBackupCurrentApp = this.config.noBackupCurrentApp;
    ...
    async.mapSeries(
      ...
      function (sessionData, callback) {
        ...
        var taskList = taskListsBuilder.deploy(
          bundlePath, env,
          deployCheckWaitTime, appName, enableUploadProgressBar, enableSharpBinaryFix, noBundleDelete, noBackupCurrentApp);
        taskList.run(session, function (summaryMap) {
          callback(null, summaryMap);
        });
      },
      whenAfterDeployed(buildLocation, noBundleDelete, noBackupCurrentApp)
    );
  }
  ...
  if (!prebuiltBundle) {
    ...
    console.log('Deploying prebuilt bundle: ' + prebuiltBundle);
    bundlePath = prebuiltBundle;
    buildLocation = path.dirname(prebuiltBundle)
    deployFunc();
  }
};
...
function whenAfterDeployed(buildLocation, noBundleDelete, noBackupCurrentApp) {
  return function(error, summaryMaps, noBackupCurrentApp) {
    if (!noBundleDelete || noBundleDelete === "no") {
	    rimraf.sync(buildLocation);
    } else {
	    console.log('noBundleDelete is enabled - not removing bundle after deploy'.bold.red);
    }
    if (noBackupCurrentApp || noBackupCurrentApp === "yes") {
      console.log('noBackupCurrentApp is enabled - not backing up current app'.bold.red);
    }
    whenAfterCompleted(error, summaryMaps, noBackupCurrentApp);
  };
}
